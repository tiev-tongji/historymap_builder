...
//LOG(INFO)<<msg->nsec<<","<<msg->angle_heading<<std::endl;
...
void * PerceptionNode::PublishNavInfo(void* __this){
    ...
    // cartographer输出第一个slam结果后才开始正常输出nav_info
        ...
        nav_mtx.lock();
        //msg_nav_temp.timestamp = (GpsAndSystemDiffSeconds + gps_temp_nav.week * SecondsPerWeek + gps_temp_nav.time - 18) * 1000000;
        msg_nav_temp.timestamp = ((static_cast<int64_t>(GpsAndSystemDiffSeconds + gps_temp_nav.week * SecondsPerWeek) - 18) * static_cast<int64_t>(100) +
                                   static_cast<int64_t>(gps_temp_nav.time * 100.0 + 0.5) ) * static_cast<int64_t>(10000);
...
void * PerceptionNode::GenerateFusionMap(void* __this)
{
    ...
    while (!BoolStopMapping) {
        ...
        //TODO utm转local坐标系
	std::cout << historical_map.utm_x << ", " << historical_map.utm_y << std::endl;
...
//        LOG(INFO) << yaw << std::endl;
	...
	if(historical_map.utm_x - _this->initial_translation[0] < 540 + 25 and 540 - 25 < historical_map.utm_x - _this->initial_translation[0]
           and historical_map.utm_y - _this->initial_translation[1] < 60 + 60 and historical_map.utm_y - _this->initial_translation[1] > 60 - 70){
            for(int i = 0;i<GRID_ROW;i++){
                for(int j = 0;j<GRID_COL;j++){
                    historical_map.cells[i][j] = 0;
                }
            }
        }

        if(historical_map.utm_x < 288878.307 + 10 and 288878.307 - 10 < historical_map.utm_x and historical_map.utm_y < 3497489.420 + 10
                and historical_map.utm_y > 3497489.420 - 10 ){
            for(int i = 0;i<GRID_ROW;i++){
                for(int j = 0;j<GRID_COL;j++){
                    historical_map.cells[i][j] = 0;
                }
        ...
        if(historical_map.utm_x < 288734.701 + 20 and 288734.701 - 20 < historical_map.utm_x and historical_map.utm_y < 3497470.977 + 20
           and historical_map.utm_y > 3497470.977 - 20 ){
            for(int i = 0;i<GRID_ROW;i++){
                for(int j = 0;j<GRID_COL;j++){
                    historical_map.cells[i][j] = 0;
                }
        ...
        if(historical_map.utm_x < 289700.619 + 50 and 289700.619 - 50 < historical_map.utm_x and historical_map.utm_y < 3497211.064 + 50
           and historical_map.utm_y > 3497211.064 - 50 and historical_map.utm_x < 289567.404 + 50 and historical_map.utm_x > 289567.404 - 50
           and historical_map.utm_y < 3497347.990 + 50 and historical_map.utm_y > 3497347.990 - 50){
            for(int i = 0;i<GRID_ROW;i++){
                for(int j = 0;j<GRID_COL;j++){
                    historical_map.cells[i][j] = 0;
                }
