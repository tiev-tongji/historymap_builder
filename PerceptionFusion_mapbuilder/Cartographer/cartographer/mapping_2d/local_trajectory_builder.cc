...
LocalTrajectoryBuilder::AddAccumulatedRangeData(
  ...
  //*****zz 19-10-14*****//
  if( !extrapolator_->IsOometryDataEmpty() )
  {
    pose_estimate_2d = pose_prediction;//no scan match
  }else{
...
void LocalTrajectoryBuilder::AddOdometerData(
    const sensor::OdometryData& odometry_data) {
      //if (!options_.use_imu_data()) {
  InitializeExtrapolator(odometry_data);
      //}
  //check address
...
void LocalTrajectoryBuilder::InitializeExtrapolator(const sensor::OdometryData& odometry_data) {
  if (extrapolator_ != nullptr) {
    return;
  }
...
//      options_.imu_gravity_time_constant());
  ...
  extrapolator_->AddPose(odometry_data.time, transform::Rigid3d::Identity());
  //set originalpose removed by JOHN 2017.10
  //extrapolator_->AddOriPose(odometry_data.pose);
}
//*****zhaolaoshi 10-16*****//
