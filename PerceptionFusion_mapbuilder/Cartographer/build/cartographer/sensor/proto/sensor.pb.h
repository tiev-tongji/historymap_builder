...
class CompressedRangeData : public ::google::protobuf::Message /* @@protoc_insertion_point(class_definition:cartographer.sensor.proto.CompressedRangeData) */ {
 public:
  ...
  #if LANG_CXX11
  CompressedRangeData(CompressedRangeData&& from) noexcept
    : CompressedRangeData() {
    *this = ::std::move(from);
  }

  inline CompressedRangeData& operator=(CompressedRangeData&& from) noexcept {
    if (GetArenaNoVirtual() == from.GetArenaNoVirtual()) {
      if (this != &from) InternalSwap(&from);
    } else {
      CopyFrom(from);
  ...
  friend void swap(CompressedRangeData& a, CompressedRangeData& b) {
    a.Swap(&b);
  }
